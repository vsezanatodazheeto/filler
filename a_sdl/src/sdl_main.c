...
int main_v (t_filler *lst)
{
    ...
    SDL_Texture *m_corona = NULL;

    
	int i = 0;
	if (create(&win, &rend) == 1)
		return 1;
	if (load(&rend, &bg, &m_key, &m_filler, &m_p1, &m_p2, &m_corona) == 1)
		return 1;
    TTF_Font *sans = NULL;
    if(!(sans = TTF_OpenFont("resources/Verdana.ttf", 28)))
    {
        ft_printf("%s\n", IMG_GetError());
        ft_printf("govno\n");
    }
    ...
    while (run)
    {
        ...
        m_corona_p.x = m_p2_p.x;
        m_corona_p.y = m_p2_p.y;
        m_corona_p.w = SCREEN_WIDTH / 5 / 4;
        m_corona_p.h = BAR_HEIGHT;

        m_key_p.x = INDENT;
        m_key_p.y = (BAR_HEIGHT * 2) + (INDENT * 3);
        m_key_p.w = SCREEN_WIDTH / 5;
        m_key_p.h = KEY_HEIGHT;

        m_filler_p.x = (SCREEN_WIDTH / 5) + (INDENT * 2);
        m_filler_p.y = INDENT;
        m_filler_p.w = SCREEN_WIDTH - (SCREEN_WIDTH / 5) - (INDENT * 3);
        m_filler_p.h = BAR_HEIGHT + KEY_HEIGHT + INDENT;
        ...
        SDL_RenderCopy(rend, m_corona, NULL, &m_corona_p);
